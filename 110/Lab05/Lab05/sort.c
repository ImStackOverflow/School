...
//	    printf("%i", wordCount);
    ...
    //free list 
    sortedWords = LinkedListGetFirst(sortedWords);
    unsortedWordList = sortedWords;
    while(unsortedWordList != NULL){
	unsortedWordList = sortedWords->nextItem;
	LinkedListRemove(sortedWords);
	sortedWords = unsortedWordList;
    }
    printf("done\n");
#endif
...
int SortedWordCount(ListItem *list, int *wordcount)
{
	...
	while (butt != NULL && strcmp(list->data, butt->data) == 0) {
	    j++;
	    butt = butt->nextItem;
	}
	//put in occurences and negative numbers
	wordcount[i++] = j;
	int k;
	for (k = 0; k < j-1; k++) {
	    wordcount[i++] = -j;
