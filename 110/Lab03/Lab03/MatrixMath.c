 ...
 * Matrix adjugate
 *****************************************************************************/
void cofactor(float mat[3][3], int row, int col, float cofactor[2][2]){
    row--, col--;//decriment index cause ARRAYS START AT 0
    int i, j, k = 0, z = 0;
    for (i = 0; i < 3; i++){
	if(i == row) continue;//skip row
	for (j = 0, z = 0; j < 3; j++){
	    if(j == col) continue;
	    cofactor[k][z++] = mat[i][j];
	}
	++k;
    }
}
...
void MatrixAdjugate(float mat[3][3], float adjugate[3][3]){
    float work[2][2], hold[3][3];
    int shit = 1;
    int i, j;
    for (i = 0; i < 3; i++){
	for (j = 0; j < 3; j++){
	    cofactor(mat, i+1, j+1, work);
	    hold[i][j] = shit * Det(work);
	    shit *= -1;
	}
    }
    MatrixTranspose(hold, adjugate);
}